<template>
  <div class="bg-green">
    <div class="max-width-3 mx-auto p1 ">
      ...
      <div
        class="mx-auto border border-silver p3 bg-white mb3"
        style="height:500px">
        <ul class="m0 p0 mb3 pb1">
          <li class="mb1 table">
            <div class="h4 table">
              <span
                v-if="currentBlock"
                class="align-middle table-cell green">Current Block {{ currentBlock }}</span>&nbsp;
            </div>
          </li>
          <li
            :key="i"
            class='mb1  table'
            v-for="(activity, i) in pagedActivity">
            <div
              class='h4  table'
              v-if="activity.name === 'Registered'">
              <span class="align-middle table-cell"> Block {{ activity.blockNumber }} —</span>
              <router-link
                class="align-middle table-cell px1"
                :to="'/clovers/' + activity.data.board">
                <clv
                  :key="activity.data.board"
                  class=' small-clover no-hover'
                  :no-click="true"
                  :byte-board="activity.data.board"/>
              </router-link>
              <span
                class="align-middle table-cell"
                v-if="activity.data.newBoard"> claimed by </span>
              <span
                class="align-middle table-cell"
                v-else> flipped by </span>
              <router-link
                class="align-middle table-cell px1"
                :to="'/users/' + activity.data.newOwner">
                <span v-html="getName(activity.data.newOwner)"/>
              </router-link>
              <span
                class="align-middle table-cell"
                v-if="activity.data.newBoard">for {{ parseInt(activity.data.findersFee) }} ♧ with initial price {{ parseInt(activity.data.lastPaidAmount) }} ♧</span>
              <span
                class="align-middle table-cell"
                v-else>for {{ parseInt(activity.data.lastPaidAmount) }} ♧</span>
            </div>
            <div
              class='h4  table my2'
              v-else-if="activity.name === 'newUserName'">
              <span class="align-middle table-cell"> Block {{ activity.blockNumber }} —</span>
              <router-link
                class="align-middle table-cell px1"
                :to="'/users/' + activity.data.player">
                <span v-html="activity.data.player.slice(0,8) + '&hellip;'"/>
              </router-link>
            ...
            <div
              class='h4  table'
              v-else-if="activity.name === 'newCloverName'">
              <span class="align-middle table-cell"> Block {{ activity.blockNumber }} —</span>
              <router-link
                class="align-middle table-cell px1"
                :to="'/clovers/' + activity.data.board">
