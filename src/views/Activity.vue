<template>
  <div>
    <div class="fade-enter-active">
      <div
        :class="filtersColors"
        class="transition-delay sticky z1 border-bottom font-mono"
        style="top:46px"
        @click="toggleFilters">
        <div
          :class="{pointer: !showFilters}"
          class="col-12 transition-all">
          <div class="relative px2">
            <div class="h-header col-12 flex items-center justify-between">
              ...
              <div class="col-6 center">
                ...
                <span
                  v-if="maxPage"
                  class=""> of {{ maxPage }}</span>
              </div>
              <div
                v-if="showFilters"
                class="col-3 align-right pointer"
                @click.stop="addNew">
                <span>Clear</span>
              </div>
              <div
        ...
        <transition name="fade">
          <div
            v-if="showFilters || filtersVisible"
            :class="{'absolute border-bottom': !showFilters}"
            class="col-12 bg-inherit"
            @click.stop>
            <div class="sm-flex">
              <div class="sm-col-6 p2 flex items-center border-top border-top-dotted">
                ...
                <div class="center select">
                  <select v-model="filters.filter">
                    <option
                      v-for="(val, key) of types"
                      :key="key"
                      :value="key !== 'all' ? key : undefined">{{ val }}</option>
                  </select>
                </div>
              </div>
              <div class="sm-col-6 p2 flex sm-justify-end items-center border-top border-top-dotted">
                ...
                <button
                  :class="{'border': !filters.asc}"
                  class="px2 py1 pointer"
                  @click="filters.asc = false">
                  <span>Newest</span>
                </button>
                <button
                  :class="{'border': filters.asc}"
                  class="ml3 px2 py1 pointer"
                  @click="filters.asc = true">
                  <span>Oldest</span>
                </button>
              </div>
            </div>
      ...
      <div
        ...
        <div class="mx-auto bg-white">
          ...
          <div
            v-if="liveLogs.length"
            class="list-reset font-mono center"
            style="top:93px">
            <span
              class="pl2 pointer h5"
              @click.self="addNew">Show {{ liveLogs.length }} new log(s)</span>
          </div>
          ...
          <nav
            v-if="prevPossible || nextPossible"
            class="list-reset border-green flex h5 green">
            <li
              v-if="prevPossible"
              class="col-6 flex-grow pointer px2 py4 center"
              @click="filters.page--">
          ...
          <ul class="m0 p0 list-reset">
            ...
            <li
              ...
              :key="log.id || log.transactionHash"
              class="border border-dashed rounded mb2">
          ...
          <nav
            v-if="prevPossible || nextPossible"
            class="list-reset flex h5 green">
            <li
              v-if="prevPossible"
              class="col-6 flex-grow pointer px2 py4 center"
              @click="filters.page--">
          ...
          <div
            v-else
            class="center h5 font-mono h-bttm-bar px2 py3">
